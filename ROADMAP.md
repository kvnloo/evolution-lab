...
Revision binds to git SHA via `roadmap.yml`. Contributors **do not** rewrite this file. Issues labeled `needs-discussion` are not claims. Maintainers promote to `claimable`. Worker PRs follow `.verified-oss-loop/rollout.yml` (scheme `rolling`: day-pass → `preview`). This P0 lock is a maintainer PR at `main`.
